...
dt <- filter(data, !Country %in% c("Singapore", "Brunei")) # exclude Singapore due to lack of the DV, ODA; exclude Brunei for no regime poly data
...
df <- dt_all %>%  dplyr::select("Country","Year", "log_ODA", "ODA_Million", "lag_log_OECD", "log_ODAOOF", "ODA_OOF", "log_OFALL", "lag_log_ADB_Loans", "Offi_Fin_All","lag_Imp.Percent.CN", "lag_Exp.Percent.CN", "lag_Resolution", "lag_Dispute", "lag_AIIB_Member", "lag_BRI_Member","lag_UNSC", "lag_log_GDPpc",  "lag_log_Pop", "lag_Civil_Conflict", "lag_Law", "lag_Poly" ,"SCS_Border", "SCS_Features")
```
...
library(huxtable)
library(psych)
summary_stats <- summary(df[, c("log_OFALL", "lag_Imp.Percent.CN", "lag_Exp.Percent.CN", "lag_Dispute", "lag_Resolution", "lag_BRI_Member", "lag_AIIB_Member", "lag_UNSC", "lag_log_GDPpc", "lag_log_Pop", "lag_Civil_Conflict", "lag_log_OECD", "lag_log_ADB_Loans", "lag_Poly")]) 

describe_stats <- describe(df[, c("log_OFALL", "lag_Imp.Percent.CN", "lag_Exp.Percent.CN", "lag_Dispute", "lag_Resolution", "lag_BRI_Member", "lag_AIIB_Member", "lag_UNSC", "lag_log_GDPpc", "lag_log_Pop", "lag_Civil_Conflict", "lag_log_OECD", "lag_log_ADB_Loans", "lag_Poly")]) 
...
```{r}
pool_trade <- lm(log_OFALL ~ lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly +lag_log_OECD + lag_log_ADB_Loans, data = df) # baseline # drop  SCS_Border + SCS_Features
summary(pool_trade)
```
#### Trade VIF
```{r}
...
#### dispute VIF
```{r}
pool_disp <- lm(log_OFALL ~ lag_Dispute + lag_Resolution + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans, data = df) # + SCS_Border + SCS_Features
summary(pool_disp)
```
...
#### Institutions
```{r}
pool_ins1 <- lm(log_OFALL ~ lag_BRI_Member + lag_AIIB_Member + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans, data = df) # + SCS_Border + SCS_Features
summary(pool_ins1)
```
...
InterClass Coefficient (ICC) = 0.13. 
...
```{r}
fix_trade1 <- plm(log_OFALL ~ lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans, data = df, index = c("Country", "Year"), model = "within", effect="individual") # model="within" means fixed effects for the entity variable, country
summary(fix_trade1)
```
##dispute

```{r}
fix_disp1 <- plm(log_OFALL ~ lag_Dispute + lag_Resolution + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans, data = df, index = c("Country", "Year"), model = "within", effect="individual") 
summary(fix_disp1 ) # resolution significance
```
## institutions
```{r}
fix_inst1 <- plm(log_OFALL ~  lag_BRI_Member + lag_AIIB_Member + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans, data = df, index = c("Country", "Year"), model = "within", effect="individual") # model="within" means fixed effects for the entity variable, country
summary(fix_inst1)
```
...
```{r}
trade_int_1 <- lmer(log_OFALL ~ lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + (1 | Country), data = df, REML = FALSE )
summary(trade_int_1)
```
...
```{r}
trade_int_2 <- lmer(log_OFALL ~ lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + SCS_Border + SCS_Features +  (1 | Country), data = df, REML = FALSE )
summary(trade_int_2)
```
...
```{r}
trade_int_3 <- lmer(log_OFALL ~ lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly +  lag_log_OECD + lag_log_ADB_Loans + SCS_Border + SCS_Features + (1 | Country) + (1 | Year), data = df, REML = FALSE )
summary(trade_int_2)
```
...
trade_slop_1 <- lmer(log_OFALL ~ lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + (1 + lag_Imp.Percent.CN + lag_Exp.Percent.CN | Country), data = df, REML = FALSE)
...
```{r}
# only import
trade_slop_1 <- lmer(log_OFALL ~ lag_Imp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + (1 + lag_Imp.Percent.CN | Country), data = df, REML = FALSE )
summary(trade_slop_1)
```

```{r}
# only export
trade_slop_1 <- lmer(log_OFALL~ lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + (1 + lag_Exp.Percent.CN | Country), data = df, REML = FALSE )
summary(trade_slop_1)
...
```{r}
# Fixed Effects Model
fe_model <- plm(log_OFALL ~ lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + SCS_Border, data = df, model = "within")
...
### only level 1 predictors
```{r}
disp_int_1 <- lmer(log_OFALL ~ lag_Dispute + lag_Resolution + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + (1 | Country), data = df, REML = FALSE )
summary(disp_int_1)
```

### add level 2 predictors, SCS_Border and SCS_Features
```{r}
disp_int_2 <- lmer(log_OFALL ~ lag_Dispute + lag_Resolution + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + SCS_Border + SCS_Features + (1 | Country), data = df, REML = FALSE )
summary(disp_int_2)
```
...
```{r}
disp_int_3 <- lmer(log_OFALL ~ lag_Dispute + lag_Resolution + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + SCS_Border + SCS_Features + (1 | Country) + (1 | Year), data = df, REML = FALSE )
summary(disp_int_2)  
...
```{r}
disp_slop_1 <- lmer(log_OFALL ~ lag_Dispute + lag_Resolution + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans+ SCS_Border + SCS_Features + (1 + lag_Dispute + lag_Resolution | Country), data = df, REML = FALSE)
...
### only level 1 predictors
```{r}
inst_int_1 <- lmer(log_OFALL ~ lag_BRI_Member + lag_AIIB_Member + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + (1 | Country), data = df, REML = FALSE )
summary(inst_int_1)
```

### add level 2 predictors, SCS_Border and SCS_Features
```{r}
inst_int_2 <- lmer(log_OFALL ~ lag_BRI_Member + lag_AIIB_Member + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans +SCS_Border + SCS_Features + (1 | Country) , data = df, REML = FALSE )
summary(inst_int_2)
```
...
```{r}
inst_int_3 <- lmer(log_OFALL ~ lag_BRI_Member + lag_AIIB_Member + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans +SCS_Border + SCS_Features + (1 | Country) + (1 | Year), data = df, REML = FALSE )
...
singular
```{r}
inst_slop_1 <- lmer(log_OFALL ~ lag_BRI_Member + lag_AIIB_Member + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans + SCS_Border + SCS_Features + (1 + lag_AIIB_Member + lag_BRI_Member | Country), data = df, REML = FALSE)
summary(inst_slop_1)
```
...
ordered_variables <- rev(c("lag_Imp.Percent.CN", "lag_Exp.Percent.CN", "lag_UNSC","lag_log_GDPpc",  
                       "lag_log_Pop", "lag_Civil_Conflict", "lag_Poly", "lag_log_OECD", "lag_log_ADB_Loans", 
...
# trade
mcamb1 <-lm(log_OFALL  ~lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans +SCS_Border + SCS_Features,  data=df_Camb)
# resolution
mcamb2 <-lm(log_OFALL  ~lag_Dispute + lag_Resolution + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans +SCS_Border + SCS_Features,  data=df_Camb)
# institutions
mcamb3 <-lm(log_OFALL ~ lag_BRI_Member + lag_AIIB_Member + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly + lag_log_OECD + lag_log_ADB_Loans +SCS_Border + SCS_Features,  data=df_Camb)
...
# trade
mind1 <-lm(log_OFALL ~lag_Imp.Percent.CN + lag_Exp.Percent.CN + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly +lag_log_OECD + lag_log_ADB_Loans +SCS_Border + SCS_Features,  data=df_ind)
# resolution
mind2 <-lm(log_OFALL ~lag_Dispute + lag_Resolution + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly +lag_log_OECD + lag_log_ADB_Loans +SCS_Border + SCS_Features,  data=df_ind)
# institutions
mind3 <-lm(log_OFALL ~  lag_BRI_Member + lag_AIIB_Member + lag_UNSC + lag_log_GDPpc + lag_log_Pop + lag_Civil_Conflict + lag_Poly +lag_log_OECD + lag_log_ADB_Loans +SCS_Border + SCS_Features,  data=df_ind)
